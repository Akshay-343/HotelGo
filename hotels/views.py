...
def hotels(request):
    ...
    for hotel in hotels_matching_features:
        ...
        if matching_room_type:
            ...
            hotel.total_cost = total_cost  # Assign total_cost directly to the Hotel instance
...
class ReserveRoomView(View):
    ...
    def post(self, request, hotel_id, *args, **kwargs):
        ...
        reservation_successful = False  # Flag to track if any reservation is successful
        ...
        if form.is_valid():
            ...
            # Check availability before processing the reservation
            for room_type in room_types:
                quantity = form.cleaned_data.get(f'quantity_{room_type.id}', 0)

                if int(quantity) > 0 and int(quantity) <= HotelRoom.objects.get(room_type=room_type).available_rooms:
                    availability = HotelRoom.objects.get(room_type=room_type)
                    availability.available_rooms -= int(quantity)
                    availability.save()

                    # Calculate cost for the current room type and quantity
...
def add_room_type(request):
    ...
    # If form submission is not valid, you might want to handle this case accordingly
    # For simplicity, redirecting to 'hotel_upload' in case of issues
    return redirect('hotel_upload')
...
def update_available_rooms(request, hotel_id):
    ...
    if request.method == 'POST':
        ...
        if room_form.is_valid():
            ...
            # Update available rooms for the specified room type
        ...
        # If the request method is not POST, redirect to the hotel detail page
        return redirect('hotel_detail', hotel_id=hotel.id)
